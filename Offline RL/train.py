...
DESCRIPTION = '_'.join(['LR', str(LEARNING_RATE), 'OPTIM', 'ADAM', 'H', 'EPOCHS', str(EPOCHS), 'BATCH_SIZE', str(BATCH_SIZE), 'SEED', str(SEED)])
...
def main():
    ...
    print(f'Training ({EPOCHS} epochs, dataset size: {len(dataset_1)}) took {run_time:.1f} secs ({(run_time/3600):.1f} h).')
...
class AverageMeter(object):
    ...
    def __str__(self):
        fmtstr = '{name} {val' + self.fmt + '} (last) ({avg' + self.fmt + '} average, {count} samples)'
